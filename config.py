...
BATTLE_DMG_PCT_MIN = 0.012     # حداقل دمیج، ۱٫۲% مکس HP حریف (قدرت خیلی کمتر از حریف) | تقویت درخواست کارفرما
BATTLE_DMG_PCT_MAX = 0.09      # حداکثر دمیج، ۹% مکس HP حریف (قدرت خیلی بیشتر از حریف) | تقویت درخواست کارفرما
...
# رول هر زمین فقط تجربهش رو تعیین می‌کنه، تعداد برداشت ثابت از جدول HARVEST_QTY میاد (ستاره نمایشی نداره)
QUALITY_TIERS = [
...
# تعداد ثابت محصول هر زمین در هر برداشت (درخواست کارفرما: بدون رنج و رندوم تعداد، خیلی مشخص)
# رول کیفیت داخلی حالا فقط تجربه برداشت رو تعیین می‌کنه، تعداد از همین جدول میاد
HARVEST_QTY = {
    "marijuana": 2,  # ماری‌جوانا
    "gharch": 2,     # قارچ
    "peyote": 2,     # پیوت
    "kratom": 2,     # کراتوم
    "khashkhash": 2, # خشخاش سیاه
    "teriak": 3,     # تریاک (درخواست کارفرما)
    # کوکائین و بذرهای افسانه‌ای هربار ۱ واحد میدن (محصول گرونه، تعدادش محدوده)
}
...
WEATHERS = {
    ...
    "moon": {
        ...
        "announce": ["شانس برداشت محصول اضافه به مقدار {p}% بالا رفت"],
        "effects": ["شانس محصول اضافه {p}%"],
    },
    "frost": {
