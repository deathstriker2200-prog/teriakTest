...
راند ۱۲ درخواست کارفرما: «قدرت کل» ((حمله - دفاع خام) / ۴، برگردونده راند ۳۷) دو طرف مقایسه میشه و درصدی نیس، مهاجم بیشتر بود برده وگرنه باخته
...
async def total_powers(session: AsyncSession, attacker: User, target: User) -> tuple[int, int, dict]:
    """
    (قدرت کل مهاجم, قدرت کل هدف, جزئیات استت خام) | مبنا = (حمله خام - دفاع خام) / ۴ هر دو طرف (راند ۳۷)
    ...
    # راند ۳۷ (درخواست کارفرما): فرمول قدرت کل مثل قبل، (حمله - دفاع) / ۴
    a_total = int(((a_atk0 - a_def0) / 4) * (1 + a_ap + watk))
    t_total = int(((t_atk0 - t_def0) / 4) * (1 + t_dp + wdef))
    return a_total, t_total, {
